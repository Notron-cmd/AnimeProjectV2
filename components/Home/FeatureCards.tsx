...
import { Bookmark, Heart, TrendingUp, Bell, ChevronDown } from "lucide-react";
...
const features = [
  ...
  { icon: Heart, label: 'Favorites Collection', desc: 'Build your personal elite collection of all-time favorite anime. Curate a showcase of top-tier titles that define your taste and share your anime identity with the community.' },
  { icon: TrendingUp, label: 'Trending & Rankings', desc: 'Stay ahead with real-time popularity data powered by global engagement. Discover what the world is watching, from rising stars to all-time masterpieces ranked by the community.' },
  { icon: Bell, label: 'Smart Alerts', desc: 'Never miss a moment. Get instant notifications when new episodes air, your favorite shows go live, or a completed series finishes — all powered by live AniList data.' },
];
        ...
        {features.map((f, i) => (
          ...
          >
            <div className="w-10 h-10 rounded-lg bg-[#c3b4fc]/10 flex items-center justify-center shrink-0">
              <f.icon className="text-[#c3b4fc] text-xl" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <h3 className="font-semibold text-sm text-white">{f.label}</h3>
                <ChevronDown className={`text-zinc-500 text-base transition-transform duration-200 ${openIndex === i ? 'rotate-180' : ''}`} />
              </div>
